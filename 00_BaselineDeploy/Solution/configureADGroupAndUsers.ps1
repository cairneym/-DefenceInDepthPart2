...
# Create three new users, one new Group and add one user to the group
...
$adminUser = New-AzureADUser -DisplayName "NDC Admin" -PasswordProfile $PasswordProfile -UserPrincipalName "adminuser@$domainName" -AccountEnabled $true -MailNickName "adminuser" 
if ($User2.AccountEnabled) {
    Write-Host -ForegroundColor Yellow "NDC Admin created successfully"
}
...
$user = Get-AzureADUser -Filter "displayName eq 'NDC Admin'"
